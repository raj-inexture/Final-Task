...
public class UpdateUserServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		out.println("firstname:" + firstname + "end");
		 ...
		 * user.setEmail(email); user.setGender(gender); user.setDob(dob);
		 * user.setPhone(phone); user.setPassword(hashedPassword);
		 * user.setSecurityquestion(securityquestion);
		 * user.setSecurityanswer(securityanswer); user.setProfilephoto(inputStream);
		 * 
		 ...
		 * address.setCity(city[addressloop]); address.setState(states[addressloop]);
		 ...
		 * 
		 * while (technologyloop < technologies.length) { TechnologiesBeanModel
		 * technology = new TechnologiesBeanModel();
		 * 
		 * technology.setUserid(user.getUserid());
		 * technology.setTechnology(technologies[technologyloop]);
		 * 
		 * int technologiesresult =
		 * userRegistration.registerUserTechnologies(technology);
		 * 
		 * technologyloop++;
		 * 
		 * } }
